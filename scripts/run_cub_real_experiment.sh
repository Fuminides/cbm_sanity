...
CONFIG_PATH="${CONFIG_PATH:-configs/cub_real_experiments.json}"
DEVICE="${DEVICE:-cpu}"
SEED="${SEED:-0}"
DRY_RUN="${DRY_RUN:-0}"
RUN_SHARED="${RUN_SHARED:-1}"
EXPERIMENT_NAME="cub_koh112_5cls_smoke"

usage() {
  cat <<'USAGE'
Usage:
  scripts/run_cub_real_experiment.sh [preset] [options]

Options:
  --device DEVICE          Device passed to Torch, e.g. cpu or cuda.
  --seed SEED              Experiment seed and output suffix.
  --cub-root PATH          Path to CUB_200_2011.
...
Environment fallbacks:
  PYTHON_BIN, CUB_ROOT, RESULTS_ROOT, CONFIG_PATH, DEVICE, SEED, DRY_RUN, RUN_SHARED
USAGE
}
